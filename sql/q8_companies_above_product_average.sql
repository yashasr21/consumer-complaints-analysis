-- Q8  Companies whose relief rate sits well above the average for the
...
WITH company_product AS (
    SELECT company,
           product,
           COUNT(*)                             AS complaints,
           1.0 * SUM(monetary_relief) / COUNT(*)       AS rate
    FROM complaints
    GROUP BY company, product
    HAVING COUNT(*) >= 200
),
product_avg AS (
    SELECT product, 1.0 * SUM(monetary_relief) / COUNT(*) AS product_rate
    FROM complaints
    GROUP BY product
)
SELECT cp.company,
       cp.product,
       cp.complaints,
       ROUND(100.0 * cp.rate, 1)                       AS relief_rate_pct,
